...
def test_sync_is_idempotent_by_row_index(sheet_1, op_alice):
    ...
    lead1, outcome1 = lead_create_from_sheet_row(sheet_source=sheet_1, row_index=5, raw_row=row)
    lead2, outcome2 = lead_create_from_sheet_row(sheet_source=sheet_1, row_index=5, raw_row=row)
    assert lead1.id == lead2.id
    assert outcome1 == "created"
    assert outcome2 == "resynced"
...
def test_sync_normalizes_phone(sheet_1, op_alice):
    ...
    lead, _ = lead_create_from_sheet_row(sheet_source=sheet_1, row_index=2, raw_row=row)
...
def test_sync_unbound_alias_marks_needs_review(sheet_1, op_alice):
    ...
    lead, _ = lead_create_from_sheet_row(sheet_source=sheet_1, row_index=3, raw_row=row)
    assert lead.needs_review is True
...
def test_sync_unknown_alias_persists_placeholder(sheet_1):
    ...
    lead, _ = lead_create_from_sheet_row(sheet_source=sheet_1, row_index=4, raw_row=row)
    assert lead.needs_review is True
...
def test_sync_bitrix_archive_ignores_auto_assign_and_stores_metadata(sheet_bitrix, op_alice):
    ...
    lead, _ = lead_create_from_sheet_row(sheet_source=sheet_bitrix, row_index=2, raw_row=row)
