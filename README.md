...
Персональный разбор вчерашней работы оператора, сгенерированный ИИ:
- **Сбор фактов:** Сбор метрик (продажи, диалоги, пропущенные колбеки, конверсия лидов) и 3-5 выдержек из вчерашних диалогов с клиентами.
- **ИИ-генерация:** Создание резюме вчерашнего дня, сильных сторон и 3 конкретных советов с цитатами из диалогов. При невалидном JSON от LLM делается 1 автоматический retry с тем же промптом; если и второй ответ мусор — попытка помечается как `error` и следующий тик таймера её подберёт.
- **Доставка в Telegram:** Каждый день в 07:30 по Ташкенту отправляется личное сообщение с краткой сводкой вчерашнего дня, фокусом (micro-lesson) на сегодня и ссылкой на полный разбор в веб-интерфейсе. На transient-ошибках aiogram (`TelegramNetworkError`, `TelegramServerError`, `TelegramRetryAfter`) команда делает до 3 попыток с экспоненциальным бэкоффом 1s/2s/4s; на hard-ошибках (`TelegramForbiddenError`, `TelegramBadRequest`) — сразу пишет в лог и идёт дальше без retry. Если все попытки провалились, `delivered_at` не проставляется и следующий запуск таймера подберёт непосланное.
- **API и Фронтенд:** Операторы видят свой сегодняшний разбор и историю за 30 дней. Тимлиды и менеджеры видят историю разборов любого оператора на странице его деталей.
- **Настройка (opt-out):** оператор может отключить рассылку в веб-профиле («Мой профиль» → «Уведомления» → toggle «Получать ежедневный разбор дня»). Toggle шлёт `PATCH /api/me/preferences/` с булевым `daily_lesson_opt_out`. Обе management-команды (`generate_daily_lessons` и `deliver_daily_lessons`) пропускают операторов с `daily_lesson_opt_out=True`. По умолчанию рассылка включена.

Команды генерации и отправки:
```bash
# Генерация уроков (обычно в 05:30)
python manage.py generate_daily_lessons [--date YYYY-MM-DD] [--operator ID] [--dry-run]

# Отправка уроков в TG (обычно в 07:30)
python manage.py deliver_daily_lessons [--date YYYY-MM-DD] [--operator ID]
```

В продакшене обе команды запускаются systemd-таймерами (`naff-daily-lessons-generate.timer` в 05:30 и `naff-daily-lessons-deliver.timer` в 07:30, часовой пояс `Asia/Tashkent`, `Persistent=true` для отработки пропущенных запусков после ребута). Файлы юнитов и инструкция по установке — в `deploy/README.md`.
